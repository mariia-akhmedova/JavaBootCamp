...
public class CustomExceptionActivityTest extends Exception {
    public static void main(String[] args) {
        CustomExceptionActivityTest custom = new CustomExceptionActivityTest();
        try {
            custom.validateUser("John");
            custom.validateUser("Sarah");
...
//flag stores 1 if a match is found else it should remain 0 
        int flag = 0;
        for (int i = 0; i < 4; i++) {
            String userToCheck = validUsers[i];
            if (userToCheck.equals(name)) {
                flag = 1;
            }
